...
usage_message () {
  echo """Usage:
    $PROGNAME [OPT ..]
      -u | --users)    ... specify a user file (default: cluster-admins)
...
main () {
  ...
  execute "oc whoami"
  execute "oc adm groups new cluster-admins"
  execute "oc adm policy add-cluster-role-to-group cluster-admin cluster-admins"

  while read user; do
    execute "oc adm groups add-users cluster-admins ${user}"
