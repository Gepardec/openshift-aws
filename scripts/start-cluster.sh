...
main () {
  ...
  ####
  # CORE LOGIC
  local instances=$(aws ec2 describe-instances --filters "Name=tag:Name,Values=${clustername}-*" "Name=instance-state-name,Values=stopped" --query Reservations[].Instances[*].InstanceId --region ${region} --output text | tr '\r\n' ' ')
  execute "aws ec2 start-instances --region ${region} --instance-ids ${instances}"
}
